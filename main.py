...
RECENT_CHANGES = [
    {"version": "1.5.2", "date": "2026-04-10", "status": "active", "change": "Independent bot architecture — each profile runs its own bot concurrently. Light sources fire instantly for ALL profiles. JobSpy serialized via global semaphore (1 call at a time) to prevent IP bans. Every profile gets results within seconds, no starvation."},
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    OPEN_PATHS = {"/login", "/favicon.ico", "/healthz", "/api/test-sources", "/api/debug/scrape-log", "/api/debug/sources", "/api/status"}
